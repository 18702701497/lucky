...
table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
    ...
    if (layEvent === 'detail') { //查看
        //do somehing
        layerOpen("/account/toDetail/" + accountId,'账号详情')
    } else if (layEvent === 'del') { //删除
        layer.confirm('确定删除该账号吗？', function (index) {
            layer.close(index);//先关闭删除框再删除数据比较好
            myDeleted("/account/"+accountId)
            //向服务端发送删除指令
        });
    } else if (layEvent === 'edit') { //编辑
        //do something
        layerOpen("/account/toUpdate/" + accountId,'修改账号')
        layui.form.render();
